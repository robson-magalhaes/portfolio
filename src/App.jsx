        ...
        <C.InfoArea>
          ...
          <C.Corpo>
            <C.HabilitTech className='sessaoScroll'>
              {
                Skills.map((x, key) => (
                  <C.FlipCard key={key} onMouseEnter={() => setFlip(key == flip ? null : key)}>
                    <C.ItemTech key={key} flipped={flip == key ? true : false}>
                      <C.ImgItemTech src={x.image} alt={x.nome} loading='lazy'/>
                      <p>{x.nome}</p>
                    </C.ItemTech>
                  </C.FlipCard>
                ))
      ...
      <C.SessaoExperiencia id='expProfissional'>
        ...
        <C.ContainerExperiencia>
          <C.LeftExperiencia id="screenAbrir">
            <img src={moldura} alt="moldura" className={animacaofinal ? 'escondido' : 'visivel'} loading='lazy'/>
          </C.LeftExperiencia>
                ...
                <C.BoxRight key={key}>
                  ...
                  <C.ExpDescricao expandido={expandido === key}>
                    ...
                    <div onClick={() => setExpandido(key === expandido ? null : key)} className='dropdown'>
                      <img src={iconDropdown} alt="icone de dropdown" loading='lazy'/>
                    </div>
                    <C.ExpSkill>
                      {x.skills.map((i,key) => (
